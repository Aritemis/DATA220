...
low is listed first, making it the response variable. We are interested in whether or not smoking might be predictive of a low birth weight, so I agree with this assignment of explanitory and response variables.
...
40.5% of smokers had a low birth weight child compared to 25.2% of non-smokers. More directly, more smokers than non-smokers had a child with a low birth weight.
...
The smoking population is subtracted from the non-smoking population, making it negative.
...
We shuffled to take away any association between the two variables, so now 0% is the most common level of difference.
...
Perhaps certain living conditions, such as high levels of stress, can lead to both a greater likelyhood of smoking and of having a child with a low birth weight.
...
```{r}
obs_diff2 <- diffprop(low ~ ui, data = low_ui)
obs_diff2
```
...
```{r}
sims2 <- do(2000) * diffprop(low ~ shuffle(ui), data = low_ui)
sims2
```
...
```{r}
ggplot(sims2, aes(x = diffprop)) +
    geom_histogram(binwidth = 0.05) +
    geom_vline(xintercept = obs_diff2, color = "blue")
```
...
A difference of the scale we see is unlikesly to happen by chance, which supports the idea that the two variables are likely associated.
