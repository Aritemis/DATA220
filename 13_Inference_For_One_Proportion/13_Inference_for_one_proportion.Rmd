...
```{r}
tally(~ survived, data = heartTr, margins = TRUE)

tally(~ survived, data = heartTr, margins = TRUE, format = "percent")
```
...
The sample would be the `r NROW(heartTr)` heart transplant patients from a study at Stanford University. The population of interest would probably be all heart transplant recipients.
...
$H_{0}:$ At least 50% of heart transplant recipients have had a prior surgery.

$H_{A}:$ Fewer than 50% of heart transplant recipients have had a prior surgery.
...
* Random
    - The data are from a study at Stanford and we do not have a random sample of all heart transplant recipients. Hopefully the patients from Stanford are representative of all heart transplant recipients.

* 10%
    - `r NROW(heartTr)` patients are definitely less than 10% of all heart transplant recipients.
...
The test statistic is `r prior_test_tidy$estimate`, which is the proportion of people in the sample who had a prior surgery.

```{r}
SE2 <- sqrt(0.5*(1 - 0.5)/103)

z2 <- (prior_test_tidy$estimate - 0.5)/SE2
```

The standard error is `r SE2`.
The z score is `r z2`.
...
```{r}
pdist("norm", q = z2, invisible = TRUE)
```
...
We are 95% confident that the true percentage of heart transplant recipients who had had a prior surgery is captured in the interval (`r 100 * prior_test_tidy$conf.low`%, `r 100 * prior_test_tidy$conf.high`%).
