...
```{r}
qdist("norm", p = c(.025, .975), plot = FALSE)
```

The number of standard errors enclosing 95% of the population is 1.96.
...
```{r}
qdist("norm", p = c(.005, .995), plot = FALSE)
```

The critical z score enclosing 99% of the population is 2.58.
...
This calculates the confidence interval for those who went to a public scool instead of those who went to a private school.
...
A 90% confidence indterval is narrower because we are restricting the possible values to a narrower margin that we are 5% less confident that the actual value will fall between.
...
* Random
    - It is uncertain, but the data comes from the National STEM Centre, and so is probably a random sample.

* 10%
    - 1,691 people is well below 10% of all people in the UK.
    
* Success/failure
    - There are 421 sucesses and 1270 failures in the sample population, both of which are more than 10.
...
```{r}
program <- prop.test(~ smoke, data = smoking, success = "Yes")
program_tidy <- tidy(program)
#program_tidy
program_tidy$conf.low
program_tidy$conf.high
...
We are 95% certain that the number of smokers in the UK is between 22.9% and 27.0%
