...
There are more values that are less spread out.
...
```{r}
sims_1000_50 <- do(1000) * rflip(50, prob = 0.64)
ggplot(sims_1000_50, aes(x = prop)) +
...
There are, once again, more values with a smaller range.
...
The standard deviation gets smaller and more accurate, as more common values have more of an opportunity to appear and balance out the occurances of less common values.
...
For a sample of size 50, the standard error is `r sqrt(0.64 * (1 - 0.64) / 50)`.
...
The distribution is skewed because there is only a .1 chance of success. This means that np is only 1, which is less than 10.
...
For the election example, np = 32 and n(p-1) = 18. Both of these are greater than 10, so the success/failure condition is satisfied.
...
```{r}
sims_1000_143 <- do(1000) * rflip(143, prob = 0.83)
ggplot(sims_1000_143, aes(x = prop)) +
    geom_histogram(binwidth = 0.007) +
    scale_x_continuous(limits = c(.65, 1.0),
                       breaks = seq(0, 1, 0.1))
```
...
* Random
    - Met assuming the patients sampled were suitably random

* 10%
    - Met assuming there are more than 1430 people with high cholesterol.

* Success/failure
    - Met because np = 118.7 and n(1-p) = 24.3 and both are greater than 10.
...
The mean is `r mean(sims_1000_143$heads)`. For a sample of size 143 the standard error is `r sqrt(0.83 * (1 - 0.83) / 143)`.
...
The probability of the drug being shown as effective is `r 100 - (100 * pdist("norm", q = 0.85, mean = 0.83, sd = sqrt(0.83 * (1 - 0.83) / 143)))`%.
