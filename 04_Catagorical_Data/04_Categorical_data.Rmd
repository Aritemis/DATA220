...
tally(~ race, data = birthwt, format = "proportion")
```
...
It's easier to read and interpret the labels than the numerical values.
...
About 50% of the women are white, ~15% are black, and ~35% belong to another nationality.
...
38.5% of black women and 17.9% of 'other' women smoked during their pregnancy.
...
According to the data, the percentage of women who smoked during their pregnancy varied significantly between the races. This implies that there may be some connection between the two.
...
```{r}
lwgt <- factor(birthwt$low,
                levels = c(1, 0),
                labels = c("Yes", "No"))
wgtSmk <- data.frame(lwgt, smoke)
#wgtSmk
```

```{r}
tally(lwgt ~ smoke, data = wgtSmk, margins = TRUE, format = "percent")
```

40.5% of women who smoked gave birth to a child with a low birth weight, compared to only 25.2% of women who didn't. This indicates that there is likely some association between smoking during a pregnancy and the likelyhood of a low birth weight.
...
```{r}
ggplot(wgtSmk, aes(fill = lwgt, x = smoke)) +
    geom_bar(position = "dodge")
```

The display is different, but my answer is still the same. There is significant difference in the percentage of low birth weights when the mother smoked compared to when she didn't, which is indicates that those variables are likely correlated.
</div>
...
It's easier to compare how the data is spread out in each graph when they are stacked vertically.

The syntax seems to determine if the histograms will be side by side or stacked on top of each other.
...
```{r}
ptlrace <- data.frame(ptl = birthwt$ptl, race)
```

```{r}
ggplot(ptlrace, aes(y = ptl, x = race)) +
    geom_boxplot()
```

```{r}
ggplot(ptlrace, aes(x = ptl)) +
    geom_histogram(aes(y = ..density..), binwidth = 10, boundary = 10) +
    facet_grid(race ~ .)
```

Maybe I should have chosen a different variable to get better graphs, but I've already done it.

The data shows that, regardless of race, most women have had no prior premature births. There doesn't seem to be any correlation between the two variables.
...
ggplot(smoke_race, aes(fill = smoke, x = race)) +
    geom_bar(position = "dodge") +
    labs(title = "Number of Women Who Smoked by Race",
         y = "Women",
         x = "Race")
```
...
ggplot(lwt_race, aes(x = lwt)) +
    geom_histogram(aes(y = ..density..), binwidth = 10, boundary = 100) +
    facet_grid(race ~ .) +
    labs(title = "Weight at Last Menstrual Period",
         y = "Density",
         x = "Weight (lbs)")
```
