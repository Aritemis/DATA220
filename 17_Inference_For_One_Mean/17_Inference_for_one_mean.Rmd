...
They are only employed part-time.
...
```{r}
ggplot(hsb2, aes(x = hsb2$math)) +
    geom_histogram(binwidth = 3, boundary = 10)
```

```{r}
ggplot(hsb2, aes(sample = hsb2$math)) +
    geom_qq()
```
...
The sample is 200 American high school students from the High School and Beyond survey. The population of interest would be all American high school students.
...
$H_{0}:$ The mean math score for American high school students is 50.

$H_{A}:$ The mean math score for American high school students is not 50.
...
We will use a t model with 199 degrees of freedom.
...
<div class = "answer">
    ...
    Random
        This sample is random and hopefully representative.
    10%
        There are more than 2000 American high school students.
    Nearly Normal
        The sample size is large and the data appears to be nearly normal.
...
```{r}
math_test <- t.test(~ math, data = hsb2, mu = 50)
math_test_tidy <- tidy(math_test)
t3 <- math_test_tidy$statistic
t3
```

The t score is 3.992758.
...
```{r}
pdist("t", df = math_test_tidy$parameter, 
      q = c(-t3, t3), 
      invisible = TRUE)
```
...
```{r}
P3 <- math_test_tidy$p.value
P3
```

The p value is < .001. If the mean math score for American high school students were 50 then there is less than a .1% chance of seeing data this extreme.
...
There is sufficient evidence to show that the average math score for American high school students is not 50.
...
There is the possibility of a Type I error. Perhaps the average math score for American high school students really is 50 but our data was not representative of it.
...
```{r}
math_test_tidy$conf.low
```

```{r}
math_test_tidy$conf.high
```
...
We are 95% confident that the true mean math score for American high school students is within the interval (`r math_test_tidy$conf.low`, `r math_test_tidy$conf.high`).
...
50 does not fall within the range of our confidence interval and does not make sense as a likely mean value.
...
The difference in means is only a few points and, practically speaking, is not really all that significant.
...
The large sample size makes it possible.
