...
You would expect a 50/50 split, which is different from what we see above. The simulations are different because there is an equal chance of each individual coin flip to come out as heads or tails.
...
```{r}
mean(coin_flips_20_10$prop)
```

This makes sense because it is close to what we would naturally expect.
...
The distribution would probably not be that different. We already have enough data to determine what the general distribution looks like. More data would look roughly the same.
...
It is unimodal, symmetric, and has no obvious outliers. The shape probably looks like this because the expected proportion of heads is .50.
...
There have been enough cases for the distribution to even out over time.
...
```{r}
survey_2000_100 <- do(2000) * rflip(100, prob=0.85)
```

```{r}
ggplot(survey_2000_100, aes(x = prop)) +
    geom_histogram(binwidth = 0.01, boundary = 0.5)
```
...
```{r}
survey_2000_1000 <- do(2000) * rflip(1000, prob=0.85)
```

```{r}
ggplot(survey_2000_1000, aes(x = prop)) +
    geom_histogram(binwidth = 0.005, boundary = 0.5)
```
...
The distribution is more pronounced and there is less variance.
...
The range on the x axis gets smaller.
