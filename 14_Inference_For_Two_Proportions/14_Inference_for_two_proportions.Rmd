...
ulcer_sex <- data.frame(ulcer, sex)
```
...
```{r}
tally(ulcer ~ sex, data = ulcer_sex, margins = TRUE)
```

```{r}
tally(ulcer ~ sex, data = ulcer_sex, margins = TRUE, format = "percent")
```
...
The samples are 126 female patients and 79 male patients from Denmark who have malignant melanoma. Our populations of interest could be all females and males with malignant melanoma, but should probably be restricted to all males and females in Denmark with malignant melanoma to make sure are samples are more representative.
...
$H_{0}:$ There is no difference in the rate at which women and men in Denmark with malignant melanoma are also diagnosed with an ulcer.

$H_{A}:$ There is a difference in the rate at which women and men in Denmark with malignant melanoma are also diagnosed with an ulcer.
...
* Random
    - We don't know how these samples were obtained. Hopefully the 126 female patients and 79 male patients are representative of other Danish patients with malignant melanoma.

* 10%
    - The number of people in Denmark who suffer from malignant melanoma is probably more than 1260 females and 790 males.

* Success/Failure
    - There are 47 females and 43 males who meet the success condition and 79 females and 36 males who do not, all of which are larger than 10.
...
```{r}
ulcer_sex_test <- prop.test(ulcer ~ sex, data = ulcer_sex)
ulcer_sex_test_tidy <- tidy(ulcer_sex_test)
ulcer_sex_test_tidy
```


```{r}
SE2 <- sqrt(ulcer_sex_test_tidy$estimate1 *
               (1 - ulcer_sex_test_tidy$estimate1)/126 +
           ulcer_sex_test_tidy$estimate2 * 
               (1 - ulcer_sex_test_tidy$estimate2)/79)

z2 <- (ulcer_sex_test_tidy$estimate1 - ulcer_sex_test_tidy$estimate2)/SE2
```

The test statistic is `r ulcer_sex_test_tidy$estimate1 - ulcer_sex_test_tidy$estimate2`, the standard error is `r SE2`, and the z score is `r z2`.
...
```{r}
pdist("norm", q = c(-z2, z2), invisible = TRUE)
```
...
```{r}
P2 <- 2 * pdist("norm", q = z2, plot = FALSE)
P2
```

The P-value is `r P2`. If there were truly no difference between the rate at which women and men in Denmark with malignant melanoma develop ulcers then there is only a `r 100 * P2`% chance of seeing a difference in our data at least as extreme as what we saw.
...
There is sufficient evidence to suggest that there is a difference between the rate at which women and men in Denmark with malignant melanoma develop ulcers.
...
As we rejected the null there is a possibility we made a Type I error, in which case there would actually be no difference between the rate at which women and men in Denmark with malignant melanoma develop ulcers even though our samples showed a significant difference.
...
Conditions are the same for the normal model.
...
```{r}
ulcer_sex_test_tidy$conf.low

ulcer_sex_test_tidy$conf.high
```
...
We are 95% confident that the true difference between the rate at which women and men with malignant melanoma develop ulcers is captured in the interval (`r 100 * ulcer_sex_test_tidy$conf.low`%, `r 100 * ulcer_sex_test_tidy$conf.high`%).
...
The confidence interval does not contain 0, which was our null. Therefor it supports our rejection of the null.
