...
```{r}
tlm <- lm(loss ~ hard, data = Rubber)
```

```{r}
tlmt <- tidy(tlm, conf.int = TRUE)
tlmt
```
...
```{r}
ggplot(Rubber, aes(y = loss, x = hard)) +
    geom_point()
```

```{r}
ggplot(tlma, aes(x = .std.resid)) +
    geom_histogram(binwidth = 0.2, boundary = 0)
```

```{r}
ggplot(tlma, aes(sample = .std.resid)) +
    geom_qq()
```

```{r}
ggplot(tlma, aes(y = .std.resid, x = hard)) +
    geom_point() +
    geom_hline(yintercept = 0)
```

    Random
        Hopefully our data is random.
    10%
        30 should be much, much less than 10% of all tires.
    Linear association
        The scatterplot shows a clear linear relationship between rubber loss and hardness.
    Outliers
        There are no apparent outliers in the scatterplot.
    Patterns in the residuals
        The data is bimodal but not problematic.
...
The loss decrements by about 5.3 units for every 1 unit increase in hardness. A literal interpretation of the intercept would be that the loss is 550.4 when the hardness is 0, but this interpretation is not appropriate. There is no such thing as a hardness of 0.
...
R^2 = 0.54, which means that 54% of the variability in loss can be explained by variability in hardness.
...
The sample is 30 sets of tires and their data from the Rubber dataset. The population would be all tires.
...
$H_{0}:$ There is no relation between a tire's loss of mass and its hardness.

$H_{A}:$ There is a relation between a tire's loss of mass and its hardness.
...
We will use the t distribution with 28 degrees of freedom.
...
The p value is less than .001. If there were no relation between tire hardness and loss of mass then there is less than a .1% chance of seeing data this extreme.
...
There is sufficient evidence to conclude that there is a relationship between tire hardness and loss of mass.
...
There is a possibility of a Type I error. Perhaps there is actually no realtion between tire hardness and loss of mass and our data was not representative.
...
We are 95% confident that the true slope when using tire hardness to predict loss of mass is captured in the interval (`r tlmt$conf.low[2]`, `r tlmt$conf.high[2]`).
...
The confidence interval does not contain zero, which supports our rejection of the null.
