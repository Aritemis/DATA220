...
```{r}
1 - pdist("norm", q = 80, mean = 100, sd = 16, plot = FALSE)
```

The curve predicts that around 89% of people would have IQ scores over 80.
...
```{r}
pdist("norm", q = 90, mean = 100, sd = 16, plot = FALSE)
```

We would expect around 27% of people to have an IQ score under 90 based on the curve.
...
```{r}
diff(pdist("norm", q = c(112, 132), mean = 100, sd = 16, plot = FALSE))
```

We would expect about 20% of people to have IQ scores between 112 and 132 based on the curve.
...
```{r}
qdist("norm", p = 0.95, mean = 100, sd = 16, plot = FALSE)
```

We would expect the cutoff bounds for the highest 5% to be around 126.3177 based on the model.
...
```{r}
qdist("norm", p = 0.30, mean = 100, sd = 16, plot = FALSE)
```

The model predicts a cutoff bounds of 91.60959 for the lowest 30%.
...
```{r}
qdist("norm", p = c(0.1, 0.9), mean = 114, sd = 14, invisible = TRUE)
```

The model predicts cutoffs for the middle 80% to be around `r qdist("norm", p = c(0.1, 0.9), mean = 114, sd = 14, invisible = TRUE)[1]` and `r qdist("norm", p = c(0.1, 0.9), mean = 114, sd = 14, invisible = TRUE)[2]`.
...
The z score for 80 is `r scale(x = 80, center = 100, scale = 16)`
...
The z score for 102 is `r scale(x = 102, center = 100, scale = 16)`
...
The z score for 130 is `r scale(x = 130, center = 100, scale = 16)`
...
The z score for Albert Einstein is `r scale(x = 160, center = 100, scale = 16)`. The model predicts that `r 100 * (pdist("norm", q = scale(x = 160, center = 100, scale = 16), mean = 100, sd = 16))`% of people will have an IQ greater than albert einstein.
