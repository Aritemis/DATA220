...
```{r}
ggplot(cabbages, aes(y = VitC, x = Cult)) +
    geom_boxplot()
```

```{r}
ggplot(cabbages, aes(x = VitC)) +
    geom_histogram(binwidth = 4, boundary = 1) +
    facet_grid(Cult ~ .)
```
...
The samples are 30 cabbages from the c39 cultivar and 30 cabbages from the c52 cultivar. The populations are all cabbages from each cultivar.
...
$H_{0}:$ The ascorbic acid content of the two cultivars is the same.

$H_{A}:$ The ascorbic acid content of the two cultivars is not the same.
...
<div class = "answer">
```{r}
c_test <- t.test(VitC ~ Cult, data = cabbages)
c_test_tidy <- tidy(c_test)
```

We will use a t model with `r c_test_tidy$parameter` degrees of freedom.
...
<div class = "answer">
    ...
    Random 
        We have no information about the randomness of our sample cabbages. Hopefully our samples are representative of their respective cultivars.
    10% 
        30 is less than 10% of all c39 cabbages and 30 is less than 10% of all c52 cabbages.
    Nearly normal
        Since each sample has a sample size of 30 we meet the condition.
...
```{r}
pdist("t", df = c_test_tidy$parameter,
      q = c(-t2, t2),
      invisible = TRUE)
```
...
```{r}
P2 <- c_test_tidy$p.value
P2
```

The p value is less than .001. If there were no difference in the ascorbic acid contents of the tho cabbage cultivars then there is less than a .1% chance of seeing data this extreme.
...
There is sufficient evidence to conclude there is a difference in the mean Vitamin C contents of c39 and c52 cabbages.
...
There is a possibility of a Type I error. Perhaps there really is no difference between the ascorbic acid contents of the two cabbage types that wasn't reflected in our data.
...
```{r}
c_test_tidy$conf.low
```

```{r}
c_test_tidy$conf.high
```
</div>
...
We are 95% confident the difference between the mean ascorbic acid contents of c39 and c52 cabbages is captured in the interval (`r c_test_tidy$conf.low`,`r c_test_tidy$conf.high`).
...
Our confidence interval does not contain the value zero, supporting our rejection of the null.
