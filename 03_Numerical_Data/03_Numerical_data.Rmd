...
Because rather than summing up the data before interpreting it looking at the median or the IQR looks at the data at specific points. Because of this, it doesn't matter much if you have a few values that are much, much larger or smaller than the rest because it has little effect on the positioning of the rest of the data.
...
This time we used the command without an additional positioning parameter, so it gave us it's default output, which consists of the most commonly used quartiles. We can specify the quartile we want by passing it as a decimal parameter like before.
...
In this data set the minimum baby weight in grams is `r min(birthwt$bwt)` and the maximum baby weight in grams is `r max(birthwt$bwt)`.
...
The graph is unimodal as it's all grouped together in a curve, skewed to the right, mostely symmetric, and has possible outliers.
...
```{r}
ggplot(birthwt, aes(x = bwt)) + geom_histogram(binwidth = 150, boundary = 70)
```

Seems to be unimodal and is mostly symmetrical. There are possible outliers on each side.
...
It is unimodal, skewed a bit to the right, and seems to be free of outliers.
...
```{r}
ggplot(faithful, aes(x = waiting)) +
    geom_histogram(binwidth = 2, boundary = 10)
```
...
I would expect the birth weight of the mother to be the explanitory variable and the birth weight of the baby to be responsive. The baby didn't yet exist at the time the mother was weighed, so it stands to reason that it is the mother's weight that might be predictive of the child's.
...
There seems to be some positive correlation between the weight of the mother and the weight of the child.
...
Once again, as it's the mother who has the child I would consider the mother's age to be potentially predictive and the weight of the child to be potentially responsive.
...
```{r}
ggplot(birthwt, aes(y = bwt, x = age)) +
    geom_point()
```
...
There doesn't seem to be much of a correlation either way.
