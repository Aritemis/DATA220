...
```{r}
ggplot(Galton_F_d, aes(x = d)) +
    geom_histogram(binwidth = 2, boundary = 0)
```

```{r}
ggplot(Galton_F_d, aes(sample = d)) +
    geom_qq()
```
...
The sample is 433 mother-daughter pairs from a study done by Francis Galton in the 1880's. The population is presumably all mothers and their daughters.
</div>
...
$H_{0}:$ There is no difference between the heights of the mothers and the heights of their daughters.

$H_{A}:$ There is a difference between the heights of the mothers and the heights of their daughters.
...
We will use a t model with 432 degrees of freedom.
...
<div class = "answer">
    ...
    Random
        We did not collect the data and do not know for sure if it is random. Hopefully it is a random and representative sample.
    10%
        The mother-daughter pairs in question are far fewer than 10% of all such pairs.
    Nearly normal
        The sample size is larger than 30 and the distribution looks fairly normal.
...
```{r}
md_test <- t.test(Galton_F_d$d, mu = 0)
md_test_tidy <- tidy(md_test)
t2 = md_test_tidy$statistic
```

The test statistic is `r md_test_tidy$statistic`.
...
```{r}
pdist("t", df = md_test_tidy$parameter,
      q = c(-t2, t2),
      invisible = TRUE)
```
...
```{r}
P2 <- md_test_tidy$p.value
P2
```

The p value is `r P2`. If there is no difference in the heights of mothers and their daughters then there is a `r 100 * P2`% chance of seeing data this extreme.
...
The is insufficient evidence to show that there is a difference between the heights of mothers and their daughters.
...
There is a possibility of a Type II error. Perhaps there is a difference in the heights of mothers and their daughters that our data did not show.
...
```{r}
md_test_tidy$conf.low
```

```{r}
md_test_tidy$conf.high
```
...
We are 95% confident that the true difference in inches of the heights of mothers and their daughters when subtracting mother height from daughter height is captured in the interval (`r md_test_tidy$conf.low`, `r md_test_tidy$conf.high`).
...
The value 0 is included in our confidence interval, meaning that 0 is a plausible value for the difference, which supports our conclusion.
