...
```{r}
smoke_status_tally <- tally(smoke ~ maritalStatus, data = smoking)
smoke_status_tally
```
...
The sample is 1691 people from the United Kingdom. The population of interest would be people from the UK.
...
$H_{0}:$ There is no relation between smoking habits and marital status.

$H_{A}:$ There is a relation between smoking habits and marital status.
...
We will use a chi-square model with 4 degrees of freedom.
(rows - 1) * (colums - 1)
...
<div class = "answer">
    ...
    Random
        Hopefully the people in this study were randomly sampled and representative of all people from the UK, but we don't know for sure.
    10%
        There are more than 16910 people in the UK.
    Expected cell counts
        All expected cell values are greater than 5.
```{r}
smoke_status_test <- chisq.test(smoke_status_tally)
smoke_status_test$expected
```
...
```{r}
smoke_status_test_tidy <- tidy(smoke_status_test)
```

The value of $\chi^{2}$ is `r smoke_status_test_tidy$statistic`.
...
```{r}
pdist("chisq", df = smoke_status_test_tidy$parameter,
      q = smoke_status_test_tidy$statistic,
      invisible = TRUE)
```
...
```{r}
P2 <- 1 - pdist("chisq", df = smoke_status_test_tidy$parameter,
      q = smoke_status_test_tidy$statistic,
               plot = FALSE)
```

The P-value is < .001. If smoking and marital status were really independent then there would be less than a .1% chance of seeing results at least as extreme as our data.
...
There is sufficient evidence that smoking habits and marital status are related.
...
There is the possibility of a Type I error. Perhaps our data is not representative and there isn't actually an association between smoking and marital status.
...
```{r}
smoke_status_test$residuals
```

There are fewer married and more single smokers than we expected.
