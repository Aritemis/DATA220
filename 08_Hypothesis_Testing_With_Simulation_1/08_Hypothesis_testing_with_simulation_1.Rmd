...
```{r}
ui <- factor(birthwt$ui, levels = c(1, 0), labels = c("Yes", "No"))
low_ui <- data.frame(low, ui)
low_ui
```
...
```{r}
tally(low ~ ui, data = low_ui, margins = TRUE)
tally(low ~ ui, data = low_ui, margins = TRUE, format = "percent")
```
...
The samples are 28 mothers who have ui and 161 mothers who do not. The population is mothers who gave birth at this particular hospital.
...
$H_{0}:$ There is no connection between ui and low birth weight.

$H_{A}:$ Ui and low birth weight are associated.
...
Random
  The samples are of mothers with and without ui from the Baystate Medical Center. They are hopefully random, representative samples of all the mothers who gave birth there, but there is no evidence of this.
  
10%
  The 28 mothers with ui and the 161 without could hardly add up to 10% of the mothers who have ever given birth at the Baystate Medical Center.
...
```{r}
obs_diff <- diffprop(low ~ ui, data = low_ui)
obs_diff
```

There was a an observed difference of -22.04969% between the proportion of low birth weight children from mothers with ui and mothers without.
...
```{r}
sims <- do(5000) * diffprop(low ~ shuffle(ui), data = low_ui)
ggplot(sims, aes(x = diffprop)) +
    geom_histogram(binwidth = 0.05) +
    geom_vline(xintercept = obs_diff, color = "blue") +
    geom_vline(xintercept = -obs_diff, color = "blue")
...
A P-value of 0.0356 means that there would be a 3.56% chance of getting data just as or more extreme than what we observed if there is no correlation between ui and a low birth weight.
...
Based on the data observed we must reject the null hypothesis.
...
There is enough evidence to imply, if not confirm, that there may be a correlation between low birth weight and the presence of uterine irritability.
...
As the null was rejected, there is a possibility of a Type I error. Our sample may have simply been unusual, leading us to wrongly conclude that there may be some connection between the two variables.
